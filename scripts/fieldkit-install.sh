...
setup_tailscale_repository() {
    if is_installed tailscale || apt_package_available tailscale; then
        return 0
    fi
...
validate_catalog() {
    local line_number=0
    local line action package name recommendation reason source extra
    ...
    while IFS= read -r line || [[ -n "${line}" ]]; do
        ...
        [[ -z "${action}" || -z "${package}" || -z "${name}" || -z "${recommendation}" || -z "${reason}" || -z "${source}" || -n "${extra}" ]] && \
        ...
        case "${source}" in
            apt|external:*) ;;
            *) fail "Invalid source '${source}' at ${CONFIG_FILE}:${line_number}" ;;
        esac
...
install_external_package() {
    ...
    case "${package}" in
        ...
        wifiman)
            ...
            if ! command -v curl >/dev/null 2>&1; then
                log "curl is required to download WiFiman; installing curl first."
                sudo apt-get install -y curl
            fi
            local temp_dir deb_file
            temp_dir="$(mktemp -d)"
            deb_file="${temp_dir}/wifiman-desktop.deb"
...
choose_packages() {
    ...
    while true; do
        ...
        case "${answer,,}" in
            ...
            a|all)
                for item in "${!packages[@]}"; do
                    if [[ "${action}" == "remove" || "${states[item]}" == "AVAILABLE" || "${states[item]}" == "EXTERNAL" ]]; then
                        selected+=("${item}")
                    fi
                done
                ;;
            r|recommended)
                for item in "${!packages[@]}"; do
                    if [[ "${action}" == "remove" ]]; then
                        ...
                        [[ "${recommendations[item]}" == "RECOMMENDED" && ( "${states[item]}" == "AVAILABLE" || "${states[item]}" == "EXTERNAL" ) ]] && selected+=("${item}")
                    fi
                done
                ;;
            *)
                ...
                for item in ${answer}; do
                    ...
                    if [[ "${action}" == "install" && "${states[item]}" != "AVAILABLE" && "${states[item]}" != "EXTERNAL" ]]; then
    ...
    if [[ "${action}" == "remove" ]]; then
        ...
        for index in "${selected[@]}"; do
            ...
            if [[ "${source}" == external:* ]]; then
                ...
                log "Installing selected APT package: ${package}"
...
# Configure external repositories only when their catalog entries are present.
for item in "${!INSTALL_PACKAGES[@]}"; do
    if [[ "${INSTALL_SOURCES[item]}" == "external:tailscale" ]]; then
        setup_tailscale_repository
        break
    fi
done
